...
def compare_images(imageA, imageB):
  ...
  return 1 - score  # Convert SSIM to dissimilarity

def compare_numpy(imageA, imageB):
  #  print(imageA.shape)
  #  print(imageB.shape)
...
def measurement_model(particlePose):
  ...
  # Downsampling source and latest images
  # source_image_downsampled = cv2.resize(source_image, (0, 0), fx=0.5, fy=0.5)
  # latest_image_downsampled = cv2.resize(latest_image, (0, 0), fx=0.5, fy=0.5)
  ...
  for offset in range(90, 10, -10):
    ...
    # source_cropped = source_image_downsampled[0:height, 0:overlap]
    # latest_cropped = latest_image_downsampled[0:height, offset:width] #downscaling
    diff = compare_images(source_cropped, latest_cropped)
    # diff = compare_numpy(source_cropped, latest_cropped)
